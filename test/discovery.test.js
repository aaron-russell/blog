...
test('astro-owned scripts opt out of Rocket Loader rewrites', async () => {
  ...
  const cookieConsent = await readFile(path.join(__dirname, '..', 'src', 'components', 'CookieConsent.astro'), 'utf8')
  const footer = await readFile(path.join(__dirname, '..', 'src', 'components', 'Footer.astro'), 'utf8')
  ...
  assert.match(baseLayout, /<CookieConsent \/>/)
  assert.match(analytics, /<script>/)
  assert.match(analytics, /window\.localStorage\.getItem\('analytics-consent'\) === 'granted'/)
  assert.match(analytics, /window\.addEventListener\('analytics-consent:granted'/)
  assert.match(analytics, /window\.addEventListener\('storage'/)
  assert.match(cookieConsent, /data-cookie-accept/)
  assert.match(cookieConsent, /data-cookie-decline/)
  assert.match(cookieConsent, /window\.dispatchEvent\(new Event\('analytics-consent:granted'\)\)/)
  assert.match(cookieConsent, /window\.localStorage\.setItem\(storageKey, value\)/)
